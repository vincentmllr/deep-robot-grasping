...
const std::string PLANNING_GROUP_HAND = "hand";
...
const double GRIPPER_WIDTH_CLOSED = 0.0;
const double GRIPPER_WIDTH_OPEN = 0.08;
const double GRIPPER_FORCE = 25.0;
const double GRIPPER_EPSILON = 0.1;
...
const float WORKSPACE_LOWER_BOUND_Z = 0.14 - 0.02 - 0.039; // floor + distance before gripper facing downwards touches tabletop
...
const std::string SAVE_IMAGES_FILEPATH = "../rosworkspaces/abschlussarbeiten/panda_deep_grasping/real_ws/src/panda-deep-grasping/replay_buffer_files/replay_images/";


/**
 * @brief Class for environment node that communicates with real hardware. Defines publishers, subscribers and clients 
 * for interaction with gripper, arm and camera and offers the reset- and the step-service. 
 */
class Environment {
  ...
  public:         
     ...
     * interaction with gripper, arm and camera and servers for the reset- and the step-service. 
     * 
    ...
    Environment(ros::NodeHandle node): move_group_interface(PLANNING_GROUP_ARM), hand_interface(PLANNING_GROUP_HAND), it(node){
      ...
      ros::service::waitForService("/move_group/get_loggers", -1);   // TODO Check if services are available (waitforservice)(moveit, etc?)

      gripper_publisher = node.advertise<control_msgs::GripperCommandActionGoal>("/franka_gripper/gripper_action/goal", 1000);
      cancel_grasp_publisher = node.advertise<actionlib_msgs::GoalID>("/franka_gripper/grasp/cancel", 1000);

      sub = it.subscribe("/camera/depth/image_rect_raw", 1, &Environment::imageCallback, this);
      gripper_status_subscriber = node.subscribe("/franka_gripper/grasp/status", 1000, &Environment::graspStatusCallback, this);

      reset_server = node.advertiseService("environment_real/reset", &Environment::resetEnvironment, this);
      ROS_INFO("environment_real/reset advertised.");
      step_server = node.advertiseService("environment_real/step", &Environment::environmentStep, this);
      ROS_INFO("environment_real/step advertised.");
...
void Environment::imageCallback(const sensor_msgs::ImageConstPtr& msg) { 
  ...
  cv_bridge::CvImagePtr image_pointer = cv_bridge::toCvCopy(msg, "");
  cv::Mat latest_image = image_pointer -> image;
  float contrast_factor_alpha = 32; 
  latest_image.convertTo(latest_image, -1, contrast_factor_alpha, 0);
  latest_image.convertTo(latest_image, -1, 1, 100); // increase brightness by 100

  // Skip every second image to lower freqency
  if(!skip_image) {
  ...
  try {
    ...
    cv::imshow("Camera View", latest_image);
    cv::waitKey(30);
...
float Environment::determine_reward(bool done, bool move_success, float gripper_height) {
  ...
  if(done) {
      ...
      // Check for successsful grasp
      float gripper_height = move_group_interface.getCurrentPose().pose.position.z;
      std::vector<double> hand_joint_values = hand_interface.getCurrentJointValues();
      float gripper_width = hand_joint_values[0] + hand_joint_values[1];
      ROS_DEBUG("Current hand joint values: %f %f", hand_joint_values[0] , hand_joint_values[1]);
      float gripper_joints_closed_width = 2 * 0.000063;
      float gripper_joints_opened_width = 2 * 0.039980; // 3.9 cm
      bool object_grasped = (gripper_height >= 0.3) && (gripper_width >= (gripper_joints_closed_width + 0.001)); // 1 mm buffer
...
/**
 * @brief Service description for reset-service. Moves robot in initial pose, waits for manual cube placement and returns the intial state.
 * 
 * @param req empty
...
bool Environment::resetEnvironment(panda_deep_grasping::ResetEnvironment::Request& req,
  ...
  // Wait so there is time for cube placement
  ROS_INFO("Please put cube in start position!");
  ros::Duration(5).sleep();
  ROS_INFO("Taking images...");
...
bool Environment::environmentStep(panda_deep_grasping::EnvironmentStep::Request& req,
  ...
  if(grasp) {
    ...
    res.done = true;
    move_to_inital_position();
    close_gripper(); // Close gripper again to see if object is still grasped
...
/**
 * @brief Initializes environment_real node, starts spinner and creates Environment object.
 * 
...
int main(int argc, char** argv) {
  ...
  ros::init(argc, argv, "environment_real");
